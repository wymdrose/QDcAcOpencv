...
namespace ParaConfig
{
	...
	bool _cmdParse(CmdSet& cmdSet, QString& msg)
	{
		...
		if (cmdSet.cmd == "DCV")
		{
			...
			else if (cmdSet.type == CHECK)
			{
				...
				msg += QStringLiteral("DCµÁ—π%0V ").arg(cmdSet.values.is);
		...
		else if (cmdSet.cmd == "ACL")
		{
			...
			else if (cmdSet.type == CHECK)
			{
				...
				msg += QStringLiteral("AC∏∫‘ÿ%0W ").arg(cmdSet.values.is);
		...
		else if (cmdSet.cmd == "ACV")
		{
			...
			msg += QStringLiteral("ACµÁ—π%0V ").arg(cmdSet.values.is);
		...
		else if (cmdSet.cmd == "ACF")
		{
			...
			msg += QStringLiteral("AC∆µ¬ %0 ").arg(cmdSet.values.is);
	...
	Error:
		msg += QStringLiteral(" ≥¨∑∂Œß[%1,%2] ").arg(cmdSet.values.tu).arg(cmdSet.values.to);
		return false;
	}
